[English Version](Softveil_ToDo_v1_3_en.md)
...
- [ ] **高度な画面加工アルゴリズム (`crabgrab` / `ScreenCaptureKit`)** 🚧
    ...
    - [x] **OCR 妨害**: スクリーンショットやカメラ撮影による文字認識を困難にする特殊テクスチャの重畳 ✅
...
- [x] **シェーダーロジックの修正・向上 (shader.wgsl)** ✅
    ...
    - [x] `OcrJammer` (mode 2) のノイズ密度と輝度を向上 ✅
...
## Phase 12.4: macOS Permission UX Optimization (Improved) 🧪 ✅

- [x] 🍎 macOS: 起動時の画面収録権限ダイアログを撤廃し、非ブロッキングな起動時チェックを導入 ✅
- [x] 🍎 macOS: 永続的な許可フラグ `screen_capture_authorized` の保存タイミングを適正化 ✅
- [x] 🍎 macOS: TCC ダイアログをトリガーしない `preflight` ベースの判定への移行 ✅

## Phase 13: Menu UX & Performance Polish 🔁 ✅

**目的:** メニュー構造の整理（モニター主導型）と、ベンチマーク更新時のメニュー閉鎖バグの解消。

- [x] **メニュー構造のモニター主導型への整理** ✅
- [x] **グローバルプリセットメニューの廃止とモニター個別管理への移行** ✅
- [x] **現在の設定をプリセットとして保存する機能の各モニターへの配置** ✅
...
## Phase 15：AI監視 (Vigilance) モードの実装と改善 🧪 ✅

**目的:** 通常時は完全な視認性を保ちつつ、カメラに複数人が映った（覗き見された）場合のみアトミックに最適なプライバシーフィルターを展開する高度防衛機能の堅牢化。

- [x] **FilterMode::AIVigilance 列挙型値의 定義** 🔁 ✅
- [x] **AppState における AI 監視起動・停止ロジックの改善** 🔁 ✅
    - [x] いずれかの有効モニターで `AIVigilance` モードが選択された際、自動的に AI 覗き見検知スレッドを起動する同期処理の実装 ✅
    - [x] すべての AI 監視モード（グローバル/個別）が解除された際、アトミックに AI 検知スレッドを停止し省電力化するロジックの実装 ✅
- [x] **描画・アニメーション同期と動的最適フィルタ適用** 🔁 ✅
    - [x] 未検出（通常時）は Alpha = 0（完全透明）に設定し、アニメーション（描画ループ）を完全停止（`ControlFlow::Wait`）させて GPU 負荷を 0% にする最適化 ✅
    - [x] 覗き見検出時は、単なる固定の `HighIntensitySPD` ではなく、**パネル種別や画面カテゴリに応じたおすすめ設定** または ユーザーが保存した**プリセット設定** を動的かつ自動的に読み込んでアトミック適用するロジックの実装 ✅
- [x] **UI/UX への統合とメニュー不具合修正 (`tray.rs`, `main.rs`)** 🔁 ✅
    - [x] 各モニターの「フィルター形式」メニューに「AI 監視 (Vigilance)」を追加 ✅
    - [x] 第一階層のAI設定を「AI 覗き見検知 ▶」親メニューに集約し、現在のモード（オフ / Vigilance / 常時フィルター強化）を動的に表示する3択サブメニューUIへ改善・完備 ✅
- [x] **ドキュメントの更新** 🔁 ✅
    - [x] 仕様書 (`Softveil_specs_v1_6.md`)、ユーザーマニュアル (`MANUAL.md`)、ToDoリストの更新 ✅

---

## Phase 16: リリース準備と品質保証 (Final) 🚀
