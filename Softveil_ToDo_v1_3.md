...
- [ ] **メニュー構造のモニター主導型への整理** 🔁
    - [ ] モニター別メニュー内への「プリセット適用」サブメニューの移動
    - [ ] モニター別メニュー内への「この画面を最適化」項目の追加
    - [ ] グローバルセクションの整理（一括適用、一括最適化、プリセット管理）
- [ ] **メニューの部分更新（Partial Update）の実装** 🔁
    - [ ] `muda::MenuItem` の参照を `TrayHandle` で保持し、全体リビルドなしでテキストを更新可能にする
    - [ ] ベンチマーク進捗（%）の更新において、メニューを閉じずにラベルのみを書き換える実装
- [ ] **個別モニター・ベンチマークのサポート** 🔁
    - [ ] `UserEvent::RunBenchmark` を拡張し、特定のモニターのみを対象にした最適化を可能にする
- [ ] **ドキュメントの更新** 🔁
    - [ ] `MANUAL.md`, `Softveil_specs_v1_6.md` のメニュー説明を最新化
