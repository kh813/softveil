...
- [ ] **プリセット用データ構造の実装 (`FilterSettings`)** 🔁
- [ ] **プリセット管理ロジックの実装 (`AppState`)** 🔁
    - [ ] ユーザー定義プリセットの保存・適用・削除機能
    - [ ] ベンチマーク結果からの「推奨プリセット（オフィス/移動中/夜間）」の自動生成
- [ ] **UI への統合 (`tray.rs`)** 🔁
    - [ ] トレイメニューへの「プリセット」サブメニューの追加
    - [ ] クイック保存・一括削除機能
- [ ] **ベンチマーク完了時の自動通知・プリセット提案** 🔁
