...
- [ ] **パネル種別 (LCD/OLED) の自動判別と適応型戦略 (NEW)** 🚧
    - [ ] 🪟 Windows: `EnumDisplayDevices` & EDID レジストリ解析の実装
    - [ ] 🍎 macOS: `system_profiler` からのモデル名抽出とキーワードマッチの統合
    - [ ] 🍎 macOS: `NSScreen` EDR 輝度による OLED/HDR 判定の統合
    - [ ] 🔁 共通: キーワード & スコアリングによるパネル種別判定ロジック
    - [ ] 🔁 共通: LCD 向けアニメーション（垂直スクロール・位相反転）の WGSL 実装
    - [ ] 🔁 共通: パネル種別に応じたフィルタプロファイルの自動適用
    - [ ] 🔁 共通: UI での判定結果表示と手動変更機能の実装
