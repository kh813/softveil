...
- [ ] **OS 設定制御ブリッジの実装** 🚧
    - [ ] **State Snapshot**: モード変更前の OS 設定（テーマ、輝度）を一時保存するロジックの実装
    - [ ] 🍎 macOS: `NSAppearance` または AppleScript によるテーマ切替、`CoreGraphics` による輝度操作の実装
    - [ ] 🪟 Windows: レジストリ (`AppsUseLightTheme`) および WMI による輝度操作の実装
    - [ ] 🔁 共通: アプリ終了時の「設定自動復元」メカニズムの構築
- [ ] **ステルス専用フィルタアルゴリズムの開発 (shader.wgsl)** 🚧
    - [ ] **Narrow Aperture Enhancement (UNA)**: OLED 向け極小サブピクセル開口ロジックのブラッシュアップ
    - [ ] **Low-Luma Contrast Collapse (LLCC)**: LCD 向け低輝度黒浮き干渉パターンの実装
- [ ] **UI/UX への統合** 🚧
    - [ ] トレイメニューの「フィルター形式」に「ステルス・ダーク」を追加
    - [ ] 自動適応: OS 側ですでにダークモードが選択されている場合、標準モードからステルス最適化フィルタへ自動移行するロジック
    - [ ] モード切替時の滑らかな輝度遷移（フェード）演出の実装
