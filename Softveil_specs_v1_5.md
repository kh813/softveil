...
- **推論エンジン**: `tract-onnx` を使用し、ローカルで ONNX モデル（Ultra-Light-Fast-Generic-Face-Detector-1MB 等）を実行する
- **カメラアクセス**: `nokhwa` を使用してクロスプラットフォームなカメラ取得を行う
- **動作フロー**:
    1. バックグラウンドスレッドで一定間隔（例: 1秒おき）にカメラフレームをキャプチャ
    2. 自分以外の顔（または2人以上の顔）を検知した場合、メインループに通知
    3. 通知を受けた際、全ディスプレイの濃度を一時的に高濃度（例: 80%）に変更
    4. 覗き見が解消されたら元の濃度設定に復元
