...
| `filter_mode` | `FilterMode` | `BlackLayer` | フィルター形式（単色 / 縦縞） |
...
パネル種別に応じて、シェーダーによる時間方向の妨害（アニメーション）を適用する。

| パネル | モード | スコアリング目安 | 特徴 |
|:---|:---|:---|:---|
| **OLED** | Static | OLED 関連キーワード、EDR > 2.0 | 黒が完全に沈むため静止画で十分。バーンインドリフト適用。 |
| **LCD IPS** | Combined | 一般的な LCD。EDR = 1.0 | バックライト漏れを抑えるため位相反転 + スクロールを併用。 |
| **LCD TN** | Vertical Scroll | TN 関連キーワード | 視野角が元々狭いため、水平方向よりも垂直方向の妨害を優先。 |
